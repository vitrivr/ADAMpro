...
sed s/HOSTNAME/$HOSTNAME/ $SPARK_HOME/adampro.conf.template > $SPARK_HOME/adampro.conf
...
$SPARK_HOME/bin/spark-submit --master "local[4]" --deploy-mode client --class ch.unibas.dmi.dbis.adam.main.Startup $SPARK_HOME/ADAMpro-assembly-0.1.0.jar &

sleep 10 #sleep until ADAMpro is started

java -jar $SPARK_HOME/ADAMpro-web-assembly-0.1.0.jar
