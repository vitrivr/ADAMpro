...
case class NormBasedDistanceFunction(n : Int) extends DistanceFunction with Serializable {
  ...
  def apply(v1: WorkingVector, v2: WorkingVector): Distance = {
    n match {
      ...
      case 2 => v1.valuesIterator.zip(v2.valuesIterator).map { case (vv1: Float, vv2: Float) => math.pow(vv1 - vv2, 2) }.reduce(_ + _)
  ...
  def apply(v1: StoredVector, v2: StoredVector): Distance = {
    v1.iterator.zip(v2.iterator).map { case (vv1: Float, vv2: Float) => math.pow(math.abs(vv1 - vv2), n) }.reduce(_ + _)
  }
