...
object IndexedRDD {
  ...
  def apply[V: ClassTag]
      (elems: RDD[(Long, V)]): IndexedRDD[Long, V] = updatable(elems)

  /**
   * Constructs an updatable IndexedRDD from an RDD of pairs, merging duplicate keys arbitrarily.
   */
  def updatable[V: ClassTag]
      (elems: RDD[(Long, V)])
    : IndexedRDD[Long, V] = updatable[V, V](elems, (id, a) => a, (id, a, b) => b)

  /** Constructs an IndexedRDD from an RDD of pairs. */
  def updatable[U: ClassTag, V: ClassTag]
      (elems: RDD[(Long, U)], z: (Long, U) => V, f: (Long, V, U) => V)
    : IndexedRDD[Long, V] = {
    val elemsPartitioned =
      if (elems.partitioner.isDefined) elems
      else elems.partitionBy(new HashPartitioner(elems.partitions.size))
    val partitions = elemsPartitioned.mapPartitions[IndexedRDDPartition[Long, V]](
      iter => Iterator(PARTPartition[Long, U, V](iter, z, f)),
