...
class SearchRPC(implicit ac : AdamContext) extends AdamSearchGrpc.AdamSearch {
  ...
  override def doProgressiveQuery(request: SimpleQueryMessage, responseObserver: StreamObserver[QueryResponseInfoMessage]): Unit = {
    ...
    try {
      val onComplete =
        ...
        QueryOp.progressive(request.entity, SearchRPCMethods.prepareNNQ(request.nnq), SearchRPCMethods.prepareBQ(request.bq), new AllProgressivePathChooser(), onComplete, request.withMetadata)
    } catch {
      case e: Exception => {
        log.error(e)
        Future.failed(e)
  ...
  override def doTimedProgressiveQuery(request: TimedQueryMessage): Future[QueryResponseInfoMessage] = {
    ...
    try {
      val (df, confidence, source) = QueryOp.timedProgressive(request.entity, SearchRPCMethods.prepareNNQ(request.nnq), SearchRPCMethods.prepareBQ(request.bq), new AllProgressivePathChooser(), Duration(request.time, TimeUnit.MILLISECONDS), request.withMetadata)
