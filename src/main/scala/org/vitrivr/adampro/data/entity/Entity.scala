...
import org.vitrivr.adampro.data.datatypes.{AttributeTypes, TupleID}
...
import org.vitrivr.adampro.utils.exception.{EntityExistingException, EntityNotExistingException, EntityNotProperlyDefinedException, GeneralAdamException}
...
case class Entity(entityname: EntityName)(@transient implicit val ac: SharedComponentContext) extends Serializable with Logging {
  private val mostRecentVersion = this.synchronized {
    if (!ac.versionManager.containsEntity(entityname)) {
      ac.versionManager.put(entityname)
    }

    ac.versionManager.getEntity(entityname)
  }
  ...
  def schema(nameFilter: Option[Seq[AttributeName]] = None, typeFilter: Option[Seq[AttributeType]] = None, fullSchema: Boolean = true): Seq[AttributeDefinition] = {
    ...
    if (_schema.isEmpty) {
      _schema = Some(ac.catalogManager.getAttributes(entityname).get.filterNot(_.pk))
    }
  ...
  def count: Long = {
    ...
    var count = ac.catalogManager.getEntityOption(entityname, Some(Entity.COUNT_KEY)).get.get(Entity.COUNT_KEY).map(_.toLong)

    if (count.isEmpty) {
      if (getData().isDefined) {
        count = Some(getData().get.count())
        ac.catalogManager.updateEntityOption(entityname, Entity.COUNT_KEY, count.get.toString)
        ac.catalogManager.updateEntityOption(entityname, Entity.APPROX_COUNT_KEY, count.get.toString)
      }
    }
  ...
  private val MAX_INSERTS_BEFORE_VACUUM = ac.catalogManager.getEntityOption(entityname, Some(Entity.MAX_INSERTS_VACUUMING)).get.get(Entity.MAX_INSERTS_VACUUMING).map(_.toInt).getOrElse(Entity.DEFAULT_MAX_INSERTS_BEFORE_VACUUM)

  /**
    * Returns the total number of inserts in entity
    */
  private def totalNumberOfInserts() = {
    ac.catalogManager.getEntityOption(entityname, Some(Entity.N_INSERTS)).get.get(Entity.N_INSERTS).map(_.toInt).getOrElse(0)
  }
  ...
  private def totalNumberOfInsertsSinceVacuuming() = {
    ac.catalogManager.getEntityOption(entityname, Some(Entity.N_INSERTS_VACUUMING)).get.get(Entity.N_INSERTS_VACUUMING).map(_.toInt).getOrElse(0)
  }
  ...
  private def incrementNumberOfInserts(i: Int = 1) = {
    ac.catalogManager.updateEntityOption(entityname, Entity.N_INSERTS, (totalNumberOfInserts + i).toString)
    ac.catalogManager.updateEntityOption(entityname, Entity.N_INSERTS_VACUUMING, (totalNumberOfInsertsSinceVacuuming + i).toString)
  }
  ...
  def insert(data: DataFrame, ignoreChecks: Boolean = false): Try[Void] = {
    ...
    try {
      ...
      val lock = ac.lockManager.getLockEntity(entityname)
      ...
      try {
        ...
        val apxCountOld = ac.catalogManager.getEntityOption(entityname, Some(Entity.APPROX_COUNT_KEY)).get.getOrElse(Entity.APPROX_COUNT_KEY, "0").toInt
        ac.catalogManager.updateEntityOption(entityname, Entity.APPROX_COUNT_KEY, (apxCountOld + insertionSize).toString)
      ...
      if (ninsertsvacuum > MAX_INSERTS_BEFORE_VACUUM || ninsertsvacuum % (MAX_INSERTS_BEFORE_VACUUM / 5) == 0 && getFeatureDataFast.isDefined && getFeatureDataFast.get.rdd.getNumPartitions > Entity.DEFAULT_MAX_PARTITIONS) {
        ...
        if (schema().filter(_.storagehandler.engine.isInstanceOf[ParquetEngine]).nonEmpty) {
          ...
          ac.catalogManager.updateEntityOption(entityname, Entity.MAX_INSERTS_VACUUMING, Int.MaxValue.toString)
        }
      }
  ...
  def indexes: Seq[Try[Index]] = {
    ac.catalogManager.listIndexes(Some(entityname)).get.map(index => Index.load(index))
  }
  ...
  def markStale(): Unit = {
    ...
    ac.catalogManager.deleteEntityOption(entityname, Entity.COUNT_KEY)
  }
  ...
  def drop(): Unit = {
    ...
    try {
      ...
      ac.catalogManager.dropEntity(entityname)
    }
  }
  ...
  /**
    ...
    */
  private def options = ac.catalogManager.getEntityOption(entityname)
  ...
  def propertiesMap(options: Map[String, String] = Map()) = {
    ...
    lb.append("indexes" -> ac.catalogManager.listIndexes(Some(entityname)).get.mkString(","))

    val apxCount = ac.catalogManager.getEntityOption(entityname, Some(Entity.APPROX_COUNT_KEY)).get.get(Entity.APPROX_COUNT_KEY)
...
object Entity extends Logging {
  ...
  def exists(entityname: EntityName)(implicit ac: SharedComponentContext): Boolean = {
    val res = ac.catalogManager.existsEntity(entityname)
  ...
  def create(entityname: EntityName, creationAttributes: Seq[AttributeDefinition], ifNotExists: Boolean = false)(implicit ac: SharedComponentContext): Try[Entity] = {
    try {
      ...
      ac.catalogManager.updateEntityOption(entityname, COUNT_KEY, "0")
      ac.catalogManager.updateEntityOption(entityname, APPROX_COUNT_KEY, "0")
      ...
      case e: Exception =>
        ...
        ac.storageManager.handlers.values.foreach {
          handler =>
            try {
              handler.drop(entityname)
            } catch {
        ...
        //drop from catalog
        ac.catalogManager.dropEntity(entityname, true)
  ...
  /**
    ...
    */
  def list(implicit ac: SharedComponentContext): Seq[EntityName] = ac.catalogManager.listEntities().get
