...
import org.vitrivr.adampro.query.ast.internal.AggregationExpression._
import org.vitrivr.adampro.query.ast.internal.BooleanFilterExpression.BooleanFilterScanExpression
import org.vitrivr.adampro.query.ast.internal.ProjectionExpression._
...
/**
  ...
  * March 2016
  */
private[communication] object MessageParser extends Logging {
  ...
  implicit def toExpression(eqm: ExpressionQueryMessage)(implicit ac: SharedComponentContext): Try[QueryExpression] = {
    try {
      ...
      val options = eqm.options

      eqm.operation match {
        case ExpressionQueryMessage.Operation.UNION => Success(UnionExpression(left.get, right.get, options, queryid))
        case ExpressionQueryMessage.Operation.FUZZYUNION => Success(FuzzyUnionExpression(left.get, right.get, options, queryid))
        case ExpressionQueryMessage.Operation.INTERSECT => Success(IntersectExpression(left.get, right.get, order, options, queryid))
        case ExpressionQueryMessage.Operation.FUZZYINTERSECT => Success(FuzzyIntersectExpression(left.get, right.get, order, options, queryid))
        case ExpressionQueryMessage.Operation.JOIN => Success(IntersectExpression(left.get, right.get, order, options, queryid))
        case ExpressionQueryMessage.Operation.EXCEPT => Success(ExceptExpression(left.get, right.get, order, options, queryid))
  ...
  /**
    ...
    * @return
    */
  def prepareNNQ(nnq: NearestNeighbourQueryMessage): Try[RankingQuery] = {
    try {
      ...
      Success(RankingQuery(nnq.attribute, fv, nnq.weights.map(prepareVector(_)), distance, nnq.k, nnq.indexOnly, nnq.options, partitions))
    } catch {
      case e: Exception => Failure(e)
    }
  }
  ...
  /**
    ...
    * @return
    */
  def prepareBQ(bq: BooleanQueryMessage): Try[FilteringQuery] = {
    try {
  ...
  def prepareInformationLevel(ilm: Seq[QueryMessage.InformationLevel]): Seq[InformationLevel] = {
    ...
    if (levels.isEmpty) {
      Seq(InformationLevels.LAST_STEP_ONLY)
    } else {
      levels
    }
  }
