...
import org.vitrivr.adampro.data.entity.{Entity, EntityNameHolder}
...
import org.vitrivr.adampro.data.index.structures.ecp.ECPIndexMetaData
...
object ECPPartitioner extends CustomPartitioner with Logging with Serializable {
  ...
  override def apply(data: DataFrame, attribute: Option[AttributeName], indexName: Option[IndexName], npartitions: Int, options: Map[String, String] = Map[String, String]())(implicit ac: SharedComponentContext): DataFrame = {
    ...
    ac.catalogManager.createPartitioner(indexName.get, npartitions, new ECPPartitionerMetaData(npartitions, leaders, indexmeta.distance), ECPPartitioner)
    //repartition
  ...
  override def getPartitions(q: MathVector, dropPercentage: Double, indexName: EntityNameHolder)(implicit ac: SharedComponentContext): Seq[Int] = {
    val meta = ac.catalogManager.getPartitionerMeta(indexName).get.asInstanceOf[ECPPartitionerMetaData]
